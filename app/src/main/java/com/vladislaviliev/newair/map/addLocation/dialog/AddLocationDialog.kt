...
import com.vladislaviliev.newair.map.addLocation.AddLocationState
...
internal fun AddLocationDialog(
    ...
    AlertDialog(
        ...
        confirmButton = {
            TextButton({ onConfirm(name) }, enabled = !state.isAdding) {
                Text("Confirm")
            }
        },
...
private fun InputField(
    ...
    TextField(
        ...
        label = { if (state.errorMessage.isNotBlank()) Text(state.errorMessage, modifier) }
    )
}
